...
if (isset($_POST['email'])) {
    ...
    $tags = $_POST['tags'];
    //securité
    //1ere verif ce compte email est deja enregistré? 
    $rq2 = "SELECT * FROM `user` WHERE email= '$email'";
        $read = $pdo->prepare($rq2);
        $read->execute();
        $result = $read->fetch();
    
    // 2eme etape hasher mon password PASSWORD_ARGON2I ou PASSWORD BCRYPT
...
<body>
    ...
    <div id="result"></div>
    <div id="titres" styles="border:1px red solide;"></div>
